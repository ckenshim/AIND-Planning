...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        self.a_levels.append(set())  # starting with empty set at the level

        for action in self.all_actions:
            a_node = PgNode_a(action)

            # if all preconditions of an action in the previous s_level, then add the action to the set
            if a_node.prenodes.issubset(self.s_levels[level]):
                # connecting to s_nodes in the prev level
    ...
    def add_literal_level(self, level):
        ...
        if level == 0:
            raise Exception("First call of 'add_literal_level' function must be at level 1.")
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # is a positive effect of action1 among the negative effects of action2
        for eff1 in node_a1.action.effect_add:
            for eff2 in node_a2.action.effect_rem:
                if eff1 == eff2:
                    return True
        # vice versa
        for eff1 in node_a1.action.effect_rem:
            for eff2 in node_a2.action.effect_add:
                if eff1 == eff2:
                    return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        '''
        # comparing effects of the first action with preconditions of the second
        for eff1 in node_a1.action.effect_add:
            for precon1 in node_a2.action.precond_neg:
                if eff1 == precon1:
                    return True
        for eff1 in node_a1.action.effect_rem:
            for precon1 in node_a2.action.precond_pos:
                if eff1 == precon1:
                    return True
        # comparing preconditions of the first action with the effects of the second
        for precon1 in node_a1.action.precond_neg:
            for eff2 in node_a2.action.effect_add:
                if precon1 == eff2:
                    return True
        for precon1 in node_a1.action.precond_pos:
            for eff2 in node_a2.action.effect_rem:
                if precon1 == eff2:
                    return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # searching for mutex parents of two A nodes
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        # comparing parents (actions) of the actions
